...
- **Configurable actions and tiered gestures (v0.5.0)**: Add, edit, or disable custom actions, each with its own hotkey, model, and system prompt. A double press runs Standard mode; a triple press can run Deep mode or route to an explicit Triple action using the same key.
